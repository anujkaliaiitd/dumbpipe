...
int main() {
  ...
  ring.ring_cyclicb.stride_bytes = 1400; // Data size, excluding headers
  ...
  profile.ring_alloc_logic = RING_LOGIC_PER_SOCKET; // Buffer per socket
  setsockopt(fd, SOL_SOCKET, SO_VMA_RING_ALLOC_LOGIC, &profile,
             sizeof(profile));

  // Bind socket to address. Currently, only MSG_DONTWAIT is supported by
  // vma_cyclic_buffer_read().
